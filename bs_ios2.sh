...
BS_CREDENTIALS=willowtree1:ztSoNK3ndGa5TjxMGZF6
...
#plutil -remove 'TestConfigurations.TestTargets.DiagnosticCollectionPolicy' ../"${FLAVOR_PREFIXED}"_iphoneos*.xctestrun

zip -r ios_tests.zip "${FLAVOR_PREFIXED}"_iphoneos*.xctestrun Release-iphoneos >/dev/null
#zip -r ios_tests.zip "${FLAVOR_PREFIXED}"_iphoneos*.xctestrun Release-iphoneos >/dev/null
#rm -rf Payload && mkdir -p Payload
#cp -r "Runner.app" Payload
#zip -r Runner.ipa Payload >/dev/null

#zip -u ios_tests.zip Runner.zip
cd - >/dev/null || exit 1
